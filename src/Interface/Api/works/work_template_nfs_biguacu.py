...
from src.Interface.Api.utils.getvalue import get_value
import json
import re
...
def work_template_nfs_biguacu(file, filename, filepathimage):
    ...
    try:
      ...
      data = {
          "cnpj_tomador": clear_blank_lines(busca_tomador_cpf_cnpj.group()),
          "razao_tomador": clear_blank_lines(busca_tomador_nome.group()),
          "insc_mun_tomador": clear_blank_lines(busca_inscricao_prestador.group()),
          "endereco_tomador": clear_blank_lines(busca_tomador_endereco.group()),
          "cnpj_prestador": clear_blank_lines(busca_prestador_cpf_cnpj.group()),
          "razao_prestador": clear_blank_lines(busca_prestador_nome.group()),
          "insc_mun_prestador": clear_blank_lines(empy_text_column),
          ...
          "descricao_atividades": clear_blank_lines(texto_sem_numeros),
          "porcentagem": clear_blank_lines(numeros[4]),
          "servicos": clear_blank_lines(empy_text_column),
          "deducoes": clear_blank_lines(array_line_desc[2]),
          "base_de_calculo": clear_blank_lines(array_line_desc[3]),
          "inss": clear_blank_lines(array_line_pis[2]),
          "iss_retido": clear_blank_lines(empy_text_column),
          "endereco_obra": clear_blank_lines(busca_endereco_obra.group()),
          "cno": clear_blank_lines(busca_line_cno.group()),
          "codigo_servico": clear_blank_lines(busca_line_servico.group()),
          "valor_total_deducoes": clear_blank_lines(empy_text_column),
          "aliquota": clear_blank_lines(empy_text_column),
          "valor_total_nota": clear_blank_lines(busca_total_bruto.group()),
          "valor_iss": clear_blank_lines(array_line_desc[4]),
          "ir": clear_blank_lines(array_line_pis[3])
      }
      
      # Convertendo o dicionário em formato JSON
      json_data = json.dumps(data)

      print("##################### json biguaçu #######################", json_data)
